...
public interface MossKillerConfig extends Config {
    ...
    @ConfigItem(
            keyName = "magicBoss",
            name = "Magic Boss",
            description = "Use Magic at boss (withdraws chaos runes)",
            position = 8,
            section = basicGuideSection
    )
    default boolean magicBoss() {
        return false;
    }
    ...
    @ConfigItem(
            keyName = "bossOnly",
            name = "Boss Only",
            description = "When not in boss mode after a while, shutdowns down rather than continues",
            position = 9,
            section = basicGuideSection
    )
