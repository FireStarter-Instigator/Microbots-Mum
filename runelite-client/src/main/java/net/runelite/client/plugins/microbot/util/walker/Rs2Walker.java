...
public class Rs2Walker {
    ...
    public static List<Tile> getLineTiles(WorldPoint from, WorldPoint to) {
        ...
        while (true) {
            LocalPoint local = LocalPoint.fromWorld(client, x1, y1);
            if (local != null) {
    ...
    public static WalkerState walkWithState(WorldPoint target, int distance) {
        ...
        // Keep this: Wait if pathfinder is still calculating
        if (ShortestPathPlugin.getPathfinder() != null && !ShortestPathPlugin.getPathfinder().isDone())
            return WalkerState.MOVING;

        // --- BUG FIX START ---
        // Only set the target if it is NEW or if the marker is missing.
        // Do NOT return MOVING here, or you skip the actual walking logic!
        ...
        // Always proceed to processWalk to actually move the player
    ...
    private static WalkerState processWalk(WorldPoint target, int distance) {
        ...
        try {
            ...
            Microbot.log("Walker Start Index: " + indexOfStartPoint + " / " + path.size());
            ...
            for (int i = indexOfStartPoint; i < path.size(); i++) {
                ...
                // Throttle intense checks to only nearby tiles (e.g. within 15 tiles)
                if (currentWorldPoint.distanceTo(Rs2Player.getWorldLocation()) < 15) {
                    doorOrTransportResult = handleDoors(path, i);
                    if (doorOrTransportResult) {
                        System.out.println("break out of door");
                        Microbot.log("Handling Door at index " + i);
                        break;
                    }
                    ...
                    if (doorOrTransportResult) {
                        ...
                        Microbot.log("Handling Rockfall at index " + i);
                        break;
                    }
                    ...
                    if (doorOrTransportResult) {
                        ...
                        Microbot.log("Handling Transport at index " + i);
                        break;
                    }
                }
                ...
                int dist = currentWorldPoint.distanceTo2D(Rs2Player.getWorldLocation());

                if (dist > nextWalkingDistance) {
                    if (Microbot.getClient().getTopLevelWorldView().isInstance()) {
                        if (Rs2Walker.walkMiniMap(currentWorldPoint)) {
                            Microbot.log("Walking to tile " + i + " (Dist: " + dist + ") via Minimap (Instance)");
                            final WorldPoint b = currentWorldPoint;
                            sleepUntil(() -> b.distanceTo2D(Rs2Player.getWorldLocation()) < nextWalkingDistance, 2000);
                            return WalkerState.MOVING; // EXIT LOOP after walking
                        }
                    } else {
                        if (dist > nextWalkingDistance) {
                            WorldPoint walkPoint = getPointWithWallDistance(currentWorldPoint);
                            if (Rs2Walker.walkMiniMap(walkPoint)) {
                                Microbot.log("Walking to tile " + i + " (Dist: " + dist + ") via Minimap");
                                final WorldPoint b = currentWorldPoint;
                                sleepUntil(() -> b.distanceTo2D(Rs2Player.getWorldLocation()) < nextWalkingDistance, 2000);
                                return WalkerState.MOVING; // EXIT LOOP after walking
                            } else {
                                Microbot.log("Minimap walk failed, trying Canvas...");
                                if (Rs2Walker.walkFastCanvas(currentWorldPoint)) {
    ...
    public static boolean walkMiniMap(WorldPoint worldPoint, double zoomDistance) {
        //Microbot.log("========== walkMiniMap START ==========");
        //Microbot.log("Target WorldPoint: " + worldPoint);

        if (worldPoint == null) {
            Microbot.log("❌ WorldPoint is null");
            return false;
        }

        // --- Player / client state ---
        //Microbot.log("Player WorldPoint: " + Rs2Player.getWorldLocation());
        //Microbot.log("Client plane: " + Microbot.getClient().getPlane());
        //Microbot.log("WorldPoint plane: " + worldPoint.getPlane());

        // --- Zoom diagnostics ---
        double currentZoom = Microbot.getClient().getMinimapZoom();
        //Microbot.log("Current minimap zoom: " + currentZoom);
        //Microbot.log("Requested minimap zoom: " + zoomDistance);

        if (currentZoom != zoomDistance) {
            Microbot.log("Setting minimap zoom...");
            Microbot.getClient().setMinimapZoom(zoomDistance);
        }

        // --- Convert World → Minimap ---
        //Microbot.log("Attempting Rs2MiniMap.worldToMinimap(...)");
        Point point = Rs2MiniMap.worldToMinimap(worldPoint);

        if (point == null) {
            Microbot.log("❌ worldToMinimap returned null");
            //Microbot.log("========== walkMiniMap END ==========");
            return false;
        }

        Microbot.log("Minimap point: x=" + point.getX() + ", y=" + point.getY());

        // --- Minimap widget diagnostics ---
        Widget minimapWidget = Rs2MiniMap.getMinimapDrawWidget();
        if (minimapWidget == null) {
            Microbot.log("❌ Minimap draw widget is null");
            return false;
        }

        Rectangle minimapBounds = minimapWidget.getBounds();
        //Microbot.log("Minimap widget bounds: " + minimapBounds);

        // --- Clip diagnostics ---
        Shape clip = Rs2MiniMap.getMinimapClipArea();
        if (clip == null) {
            Microbot.log("❌ Minimap clip area is null");
            return false;
        }

        boolean insideClip = clip.contains(point.getX(), point.getY());
        Microbot.log("isPointInsideMinimap: " + insideClip);

        if (!insideClip) {
            Microbot.log("❌ Point outside minimap clip");
...
/*
            Microbot.log("Distance from minimap center: " + distance);
            Microbot.log("Minimap approx radius: " + (minimapBounds.getWidth() / 2.0));
            Microbot.log("========== walkMiniMap END ==========");
 ...
 */
        ...
        java.awt.Point p = new java.awt.Point(point.getX(), point.getY());

        /*
        Microbot.log("Click point: " + p);
        Microbot.log("Minimap bounds: " + bounds);
        Microbot.log("Inside widget bounds: " + bounds.contains(p));
        // --- Click ---
        Microbot.log("✅ Clicking minimap at: " + point);
        Microbot.log("are we actually hotfixing");
         */
       ...
       // });
        ...
        Microbot.log("========== walkMiniMap SUCCESS ==========");
        return true;
    }
    ...
    public static int getTotalTiles(WorldPoint start, WorldPoint destination) {
        ...
        // FIXED: Explicitly use Microbot WorldArea for intersectsWith2D check
        WorldPoint lastPathPoint = path.get(path.size() - 1);
        net.runelite.client.plugins.microbot.util.api.coords.WorldArea pathArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        lastPathPoint.getX(), lastPathPoint.getY(), 2, 2, lastPathPoint.getPlane());

        net.runelite.client.plugins.microbot.util.api.coords.WorldArea objectArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        destination.getX(), destination.getY(), 2, 2, destination.getPlane());
    ...
    public static int getTotalTilesFromPath(List<WorldPoint> path, WorldPoint destination) {
        ...
        net.runelite.client.plugins.microbot.util.api.coords.WorldArea pathArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        lastPathPoint.getX(), lastPathPoint.getY(), 8, 8, lastPathPoint.getPlane());

        net.runelite.client.plugins.microbot.util.api.coords.WorldArea objectArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        destination.getX(), destination.getY(), 8, 8, destination.getPlane());
    ...
    public static boolean canReach(WorldPoint worldPoint, int sizeX, int sizeY, int pathSizeX, int pathSizeY,boolean useBankedItems) {
        ...
        // FIXED: Explicitly use Microbot WorldArea for intersectsWith2D check
        net.runelite.client.plugins.microbot.util.api.coords.WorldArea pathArea = null;
        net.runelite.client.plugins.microbot.util.api.coords.WorldArea objectArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        worldPoint.getX(), worldPoint.getY(), sizeX + 2, sizeY + 2, worldPoint.getPlane());
        ...
        try {
            ...
            WorldPoint lastPathPoint = pathfinder.getPath().get(pathfinder.getPath().size() - 1);
            pathArea = new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                    lastPathPoint.getX(), lastPathPoint.getY(), pathSizeX, pathSizeY, lastPathPoint.getPlane());
    ...
    public static int findNearestAccessibleTarget(WorldPoint startPoint, List<WorldPoint> targets, boolean useBankItems, int tolerance) {
        ...
        try {
            ...
            for (int i = 0; i < targets.size(); i++) {
                ...
                if (targetArea.intersectsWith(nearestTileArea)) {
