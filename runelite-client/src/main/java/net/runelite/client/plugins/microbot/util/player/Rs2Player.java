...
public class Rs2Player {
    ...
    /**
     * Wait for animation
     * @return true if failed to animate within 5s, false otherwise
     */
    public static boolean waitForAnimationBoolean() {
        boolean result = sleepUntilTrue(Rs2Player::isAnimating, 100, 10000);
        if (!result) {
            return true; // failed
        }
        sleepUntil(() -> !Rs2Player.isAnimating());
    ...
    public static boolean toggleRunEnergy(boolean toggle) {
        ...
        Widget widget = Rs2Widget.getWidget(InterfaceID.Orbs.RUNBUTTON);
        if (widget == null) return false;

        Rectangle bounds = widget.getBounds();

        int offsetX = Rs2Random.between(-5, 5);
        int offsetY = Rs2Random.between(-4, 4);

        int x = (int) bounds.getCenterX() + offsetX;
        int y = (int) bounds.getCenterY() + offsetY;

        Microbot.getMouse().click(x, y);
        Microbot.log(String.format("Clicked run orb at (%d, %d)", x, y));
     ...
     *
     * <p>If in the Wilderness, prioritizes blighted food items, falls back to regular food and then fast food if none are available.</p>
    ...
    public static boolean useFood() {
        List<Rs2ItemModel> allFoods = Rs2Inventory.getInventoryFood();
        if (allFoods.isEmpty()) return false;

        // 1. Get the list of IDs that count as "Fast Food"
        Set<Integer> fastFoodIds = Rs2Food.getFastFoodIds();

        // 2. Separate the inventory into "Fast" and "Standard" buckets
        List<Rs2ItemModel> fastFoodItems = allFoods.stream()
                .filter(item -> fastFoodIds.contains(item.getId()))
                .collect(Collectors.toList());

        List<Rs2ItemModel> standardFoodItems = allFoods.stream()
                .filter(item -> !fastFoodIds.contains(item.getId()))
                .collect(Collectors.toList());

        // 3. Setup Priority Logic
        boolean inWilderness = Microbot.getVarbitValue(Varbits.IN_WILDERNESS) == 1;
        Rs2ItemModel foodToUse = null;

        // --- TIER 1 & 2: Check Standard Foods ---
        if (!standardFoodItems.isEmpty()) {
            List<Rs2ItemModel> blightedFoods = standardFoodItems.stream()
                    .filter(item -> !item.isNoted() && item.getName().toLowerCase().contains("blighted"))
                    .collect(Collectors.toList());

            List<Rs2ItemModel> regularFoods = standardFoodItems.stream()
                    .filter(item -> !item.isNoted() && !item.getName().toLowerCase().contains("blighted"))
                    .collect(Collectors.toList());

            if (inWilderness && !blightedFoods.isEmpty()) {
                foodToUse = blightedFoods.get(0); // Priority 1: Blighted (if in Wildy)
            } else if (!regularFoods.isEmpty()) {
                foodToUse = regularFoods.get(0);  // Priority 2: Standard (Shark, Manta, etc.)
            }
        }

        // --- TIER 3: Fast Food (Last Resort) ---
        // Only used if we haven't picked a standard food yet
        if (foodToUse == null && !fastFoodItems.isEmpty()) {
            foodToUse = fastFoodItems.get(0);
        }
    ...
    private static Stream<Rs2PlayerModel> getPlayersMatchingCombatCriteria() {
        ...
        // Uses the Standard API WorldPoint from getWorldLocation, compatible with the fixed Rs2Pvp
        ...
        return getPlayers(player -> {
            ...
            // Ensures we pass a Standard API WorldPoint to Rs2Pvp
            int playerWildernessLevel = Rs2Pvp.getWildernessLevelFrom(player.getPlayer().getWorldLocation());
     ...
     */
    // FIXED: Returns Standard API WorldPoint
    public static net.runelite.api.coords.WorldPoint getWorldLocation() {
        if (Microbot.getClient().getTopLevelWorldView().getScene().isInstance()) {
            ...
            // This returns a standard API WorldPoint
