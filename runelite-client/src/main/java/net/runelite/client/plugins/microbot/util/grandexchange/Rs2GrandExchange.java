...
import static net.runelite.client.plugins.microbot.util.widget.Rs2Widget.clickWidgetSafe;
...
public class Rs2GrandExchange
{
	...
	public static void backToOverview()
	{
		...
		Global.sleepUntil(() -> !isOfferScreenOpen());
	}


    public static boolean anySlotContainsItem(String... searchTerms) {
        for (GrandExchangeSlots slot : List.of(
                GrandExchangeSlots.ONE,
                GrandExchangeSlots.TWO,
                GrandExchangeSlots.THREE)) {
            ...
            String slotItemName = child.getText();
            if (slotItemName == null) continue;

            for (String term : searchTerms) {
                if (slotItemName.contains(term)) {
                    Microbot.log("Found " + term + " in slot: " + slot);
	...
	public static boolean processOffer(GrandExchangeRequest request) {
		...
		switch (request.getAction()) {
			case COLLECT:
				...
				viewOffer(offerSlot);
				Global.sleepUntil(Rs2GrandExchange::isOfferScreenOpen);
			...
			case BUY:
				...
				Global.sleepUntil(GrandExchangeWidget::isOfferTextVisible);

				if (!Rs2Widget.sleepUntilHasWidgetText("Start typing the name of an item to search for it", 162, 51, false, 5000)) {
                    Microbot.log("we've missed the buy offer so we're trying again");
                    Rs2Widget.clickWidget(buyOffer);
                    Global.sleepUntil(GrandExchangeWidget::isOfferTextVisible);
                }
				...
				if (!Rs2Widget.sleepUntilHasWidgetText(searchName, 162, 43, false, 15000)) break;

				Global.sleepUntil(() -> getSearchResultWidget(request.getItemName(), request.isExact()) != null, 30000);
				...
				clickWidgetSafe(
						...
						itemResult.getRight(),
						1,
						() -> GrandExchangeWidget.getPricePerItemButton_X() != null
				);
                sleep(1900,2900);
                ...
                sleep(1900,2900);
				confirm();
				success = Global.sleepUntil(() -> !isOfferScreenOpen());
				break;
			...
			case SELL:
                ...
                Microbot.log("before processing quantity");
				if (request.getQuantity() > 0) {
                    Microbot.log("enetering processing quantity");
					setQuantity(request.getQuantity());
				}
                Microbot.log("after process quantiy and before confirm");

				confirm();
                Microbot.log("After confirm");
				success = Global.sleepUntil(() -> !isOfferScreenOpen());
				break;
		}
	...
	private static boolean collectOffer(boolean toBank)
	{
		...
		return Global.sleepUntil(() -> !isOfferScreenOpen());
	}
	...
	private static void adjustPriceByPercent(int percent)
	{
		...
		if (absPercent % 5 == 0)
		{
			...
			IntStream.range(0, times).forEach(i -> {
				...
				Global.sleepUntil(() -> GrandExchangeWidget.hasOfferPriceChanged(basePrice), 1600);
			});
		}
		else
		{
...
//					MenuEntryImpl(getOption=Customise, getTarget=, getIdentifier=2, getType=CC_OP, getParam0=15, getParam1=30474266, getItemId=-1, isForceLeftClick=false, getWorldViewId=-1, isDeprioritized=false)
				...
				Global.sleepUntil(() -> Rs2Widget.hasWidget("Set a percentage to decrease/increase"), 2000);
			...
			Rs2Widget.clickWidget(adjustXWidget);
			Global.sleepUntil(() -> GrandExchangeWidget.hasOfferPriceChanged(basePrice), 2000);
		}
	}
     ...
     */
    public static boolean buyItemExact(String itemName, int price, int quantity)
    {
	...
	{
        Microbot.log("in confirm");
		Rs2Widget.clickWidget(GrandExchangeWidget.getConfirm());
        Microbot.log("after confirm");
		Global.sleepUntil(() -> Rs2Widget.hasWidget("Your offer is much"), 2000);
	...
	private static void setQuantity(int quantity)
	{
		if (quantity != getOfferQuantity())
		{
			...
			Global.sleepUntil(() -> Rs2Widget.getWidget(InterfaceID.Chatbox.MES_TEXT2) != null); //GE Enter Price/Quantity
			sleep(1200, 2400);
	...
	private static void setPrice(int price)
	{
		if (price != getOfferPrice())
		{
			...
			Global.sleepUntil(() -> Rs2Widget.getWidget(InterfaceID.Chatbox.MES_TEXT2) != null); //GE Enter Price
            sleep(1900,2900);
	 ...
	 * @return {@code true} if the collect action was successfully initiated; {@code false} if the collect button was not found
	 */
	public static boolean collectAll1(boolean collectToBank)
	{
		if (isAllSlotsEmpty())
		{
			return true;
	 ...
	 * @return {@code true} if the collect action was successfully initiated; {@code false} otherwise
	 */
	public static boolean collectAllToBank1()
	{
		return collectAll1(true);
	}

    public static boolean collectAllToInventory1()
    {
        return collectAll1(false);
    }
    ...
    /**
     * Checks if a specific item already has a pending offer in any GE slot.
     *
     * @param itemName The name of the item to check for (partial match supported)
     * @return {@code true} if an offer exists for the item; {@code false} otherwise
     */
    public static boolean hasOfferForItem(String itemName) {
        for (GrandExchangeSlots slot : new GrandExchangeSlots[]{GrandExchangeSlots.ONE, GrandExchangeSlots.TWO, GrandExchangeSlots.THREE}) {
            // Check if slot is occupied
            if (!Rs2GrandExchange.isSlotAvailable(slot)) {
                Widget parent = GrandExchangeWidget.getSlot(slot);
                if (parent == null) continue;

                // Get the item name from child widget 19
                Widget child = parent.getChild(19);
                if (child != null) {
                    String slotItemName = child.getText();
                    if (slotItemName != null && slotItemName.contains(itemName)) {
                        Microbot.log("Found existing offer for " + itemName + " in slot " + slot);
                        return true;
                    }
                }
            }
    ...
    /**
     * Aborts a Grand Exchange offer intelligently based on slot availability and item priority.
     * <p>
     * This method dynamically fetches alchable items from the ABC Alch plugin to determine priority.
     * <p>
     * Abort priority order:
     * 1. First attempts to abort any slot containing a non-alchable item
     * 2. Then aborts consumables (bananas, chickens, bones, runes) by priority
     * 3. If all slots contain alchables, aborts any alchable slot as a fallback
     * <p>
     * After aborting, collects all items from the Grand Exchange based on the collectToBank flag.
     *
     * @param collectToBank {@code true} to collect aborted items to the bank; {@code false} to collect to inventory
     * @return {@code true} if an offer was successfully aborted and collection was initiated; {@code false} otherwise
     */
    public static boolean abortOfferSmart(boolean collectToBank)
    {
        Microbot.log("entered abort offer smart");

        // Dynamically fetch alchables from ABC Alch plugin
        Set<String> alchableNames = new HashSet<>();
        try {
            AbcAlchPlugin plugin = (AbcAlchPlugin) Microbot.getPluginManager().getPlugins().stream()
                    .filter(p -> p.getClass().getName().equals(AbcAlchPlugin.class.getName()))
                    .findFirst()
                    .orElse(null);

            if (plugin != null && plugin.getAlchItems() != null) {
                for (AlchItem item : plugin.getAlchItems()) {
                    // Only F2P profitable items with valid limits
                    if (!item.getIsMembers() && item.getGeLimit() > 0 && item.getHighAlchProfit() > 0) {
                        alchableNames.add(item.getName().toLowerCase());
                    }
                }
                Microbot.log("Loaded " + alchableNames.size() + " alchables from ABC Alch plugin");
            }
        } catch (Exception e) {
            Microbot.log("Could not load ABC Alch plugin - using fallback alchables");
        }

        // Fallback to common alchables if plugin unavailable
        if (alchableNames.isEmpty()) {
            alchableNames.addAll(Arrays.asList(
                    "steel platebody", "rune axe", "rune pickaxe", "rune mace",
                    "rune platebody", "rune longsword", "rune sword", "rune med helm"
            ));
        }

        // Check for duplicate offers
        if (Rs2GrandExchange.handleDuplicateOffers("Rune platebody", collectToBank)) {
            Microbot.log("Duplicate Rune platebody detected and handled.");
            return true;
        }

        // Try to find any non-alchable slot to abort (priority: abort non-alchables first)
        Optional<GrandExchangeSlots> nonAlchableSlot = Arrays.stream(GrandExchangeSlots.values())
                .filter(slot -> {
                    Widget parent = GrandExchangeWidget.getSlot(slot);
                    if (parent == null || Rs2GrandExchange.isSlotAvailable(slot))
                        return false;
                    ...
                    String itemName = child.getText().toLowerCase();
                    return !alchableNames.contains(itemName);
                })
                .findFirst();

        if (nonAlchableSlot.isPresent())
        {
            Widget parent = GrandExchangeWidget.getSlot(nonAlchableSlot.get());
            Widget child = parent.getChild(19);
            String itemName = child != null ? child.getText() : "Unknown";

            Microbot.log("Aborting non-alchable slot: " + itemName);
        ...
        // Priority abort list for common items (non-alchables like bananas, chickens, etc.)
        String[] abortPriority = {
                "Banana",
                "Cooked chicken",
                "Raw chicken",
                "Bones",
        ...
        for (String priorityItem : abortPriority)
        {
            Optional<GrandExchangeSlots> prioritySlot = Arrays.stream(GrandExchangeSlots.values())
                    .filter(slot -> {
                        Widget parent = GrandExchangeWidget.getSlot(slot);
                        if (parent == null || Rs2GrandExchange.isSlotAvailable(slot))
                            return false;
                        ...
                        Widget child = parent.getChild(19);
                        return child != null && child.getText().equalsIgnoreCase(priorityItem);
                    })
                    .findFirst();

            if (prioritySlot.isPresent())
            {
                Widget parent = GrandExchangeWidget.getSlot(prioritySlot.get());
                Microbot.log("Aborting by priority: " + priorityItem);
                NewMenuEntry menuEntry = new NewMenuEntry("Abort offer", "", 2, MenuAction.CC_OP, 2, parent.getId(), false);
                Rectangle bounds = parent.getBounds() != null && Rs2UiHelper.isRectangleWithinCanvas(parent.getBounds())
                        ? parent.getBounds()
                        : Rs2UiHelper.getDefaultRectangle();
        ...
        // If all slots are alchables, abort any alchable (they're all valuable)
        Optional<GrandExchangeSlots> anyAlchableSlot = Arrays.stream(GrandExchangeSlots.values())
                .filter(slot -> !Rs2GrandExchange.isSlotAvailable(slot))
                .findFirst();

        if (anyAlchableSlot.isPresent())
        {
            Widget parent = GrandExchangeWidget.getSlot(anyAlchableSlot.get());
            Widget child = parent.getChild(19);
            String itemName = child != null ? child.getText() : "Unknown";

            Microbot.log("All slots are alchables - aborting: " + itemName);
            NewMenuEntry menuEntry = new NewMenuEntry("Abort offer", "", 2, MenuAction.CC_OP, 2, parent.getId(), false);
            Rectangle bounds = parent.getBounds() != null && Rs2UiHelper.isRectangleWithinCanvas(parent.getBounds())
                    ? parent.getBounds()
                    : Rs2UiHelper.getDefaultRectangle();
        ...
        Microbot.log("No suitable slot found to abort");
        return false;
    }

    /**
     * Checks all three GE slots and aborts one offer if more than one contains the given item name.
     *
     * @param itemName      the item name to check for (case-sensitive match)
     * @param collectToBank whether to collect aborted items to the bank
     * @return true if an offer was aborted; false otherwise
     */
    public static boolean handleDuplicateOffers(String itemName, boolean collectToBank) {
        int count = 0;
        GrandExchangeSlots firstSlot = null;
        ...
        if (count > 1 && firstSlot != null) {
            Microbot.log("⚠ Detected multiple GE offers for " + itemName + ". Aborting one (" + firstSlot + ")");
            Widget parent = GrandExchangeWidget.getSlot(firstSlot);
            if (parent == null)
                return false;
    ...
    public static boolean moreThanOneSlotContainsItem(String itemName) {
        int count = 0;
        ...
        for (GrandExchangeSlots slot : List.of(
            ...
            if (slotItemName != null && slotItemName.contains(itemName)) {
                count++;
                if (count > 1) {
                    Microbot.log("More than one slot contains: " + itemName);
                    return true;
                }
            }
        }
    ...
    /**
     * Checks if any F2P Grand Exchange slot (slots 1-3) is available for a new offer.
     *
     * @return {@code true} if at least one F2P slot is available; {@code false} if all 3 slots are occupied
     */
    public static boolean isAnyF2pSlotAvailable()
    {
        return isSlotAvailable(GrandExchangeSlots.ONE) ||
                isSlotAvailable(GrandExchangeSlots.TWO) ||
                isSlotAvailable(GrandExchangeSlots.THREE);
    }
    ...
    public static boolean sellLoot(List<Integer> lootList, int percent) {
        Microbot.log("=== SELL LOOT DEBUG START ===");
        Microbot.log("Loot list size: " + lootList.size());
        Microbot.log("Percent: " + percent);

        if (Rs2Walker.getDistanceBetween(Rs2Player.getWorldLocation(),new WorldPoint(3164, 3489,0)) > 10) {
            Microbot.log("more than 10 tiles from ge, not gonna activate sell loot, walking to ge and returning false");
        ...
        // Log all items in loot list
        Microbot.log("=== LOOT LIST CONTENTS ===");
        lootList.forEach(id -> Microbot.log("Loot list contains ID: " + id));

        // Log all inventory items
        Microbot.log("=== INVENTORY CONTENTS ===");
        Rs2Inventory.items().forEach(item -> {
            Microbot.log("Inventory: " + item.getName() + " (ID: " + item.getId() + ") - Tradeable: " + item.isTradeable());
        });

        // ENHANCED DEBUGGING - Check why items are being skipped
        Microbot.log("=== DEBUGGING ITEM SKIPPING ===");
        Microbot.log("Loot list IDs: " + lootList);

        // Check each inventory item individually
        Rs2Inventory.items().forEach(invItem -> {
            boolean inLootList = lootList.contains(invItem.getId());
            boolean tradeable = invItem.isTradeable();

            Microbot.log("Item: " + invItem.getName() + " (ID: " + invItem.getId() + ")");
            Microbot.log("  - In loot list: " + inLootList);
            Microbot.log("  - Is tradeable: " + tradeable);
            Microbot.log("  - Will process: " + (inLootList && tradeable));
            Microbot.log("  ---");
        });

        // ENHANCED FILTERING with detailed logging
        Microbot.log("=== ITEM FILTERING DEBUG ===");
        List<Rs2ItemModel> allItems = Rs2Inventory.items().collect(Collectors.toList());
        List<Rs2ItemModel> filteredItems = new ArrayList<>();

        for (Rs2ItemModel item : allItems) {
            Microbot.log("Checking item: " + item.getName() + " (ID: " + item.getId() + ")");

            if (!lootList.contains(item.getId())) {
                Microbot.log("  SKIP - Not in loot list");
                continue;
            }

            if (!item.isTradeable()) {
                Microbot.log("  SKIP - Not tradeable");
                continue;
            }

            Microbot.log("  WILL PROCESS - Item passes all filters");
            filteredItems.add(item);
        }

        Microbot.log("Filtered items count: " + filteredItems.size());
        filteredItems.forEach(item -> {
            Microbot.log("Will process: " + item.getName() + " (ID: " + item.getId() + ")");
        });

        // Ensure GE is open first
        Microbot.log("=== GRAND EXCHANGE CHECK ===");
        if (!Rs2GrandExchange.isOpen()) {
            Microbot.log("Grand Exchange not open, attempting to open...");
            ...
            Microbot.log("Grand Exchange is already open");
        }

        // Process each item with detailed logging and timeouts
        for (Rs2ItemModel item : filteredItems) {
            Microbot.log("=== PROCESSING ITEM: " + item.getName() + " (" + item.getId() + ") ===");

            try {
                if (!item.isTradeable()) {
                    Microbot.log("Item not tradeable, skipping: " + item.getName());
                    continue;
                }
                Microbot.log("Item is tradeable");

                // Check for available slots and collect if needed
                Microbot.log("Checking available GE slots...");
                if (Rs2GrandExchange.getAvailableSlot() == null) {
                    Microbot.log("No available slots, collecting to bank...");
                    Rs2GrandExchange.collectAllToBank();
                    sleep(1000, 1500);

                    if (Rs2GrandExchange.getAvailableSlot() == null) {
                        Microbot.log("Still no available slots after collecting");
                        continue;
                    }
                }
                Microbot.log("Available slot found");

                // Use inventory item name directly - avoid problematic getItemDefinition call
                String itemName = item.getName();
                Microbot.log("Using inventory item name: '" + itemName + "'");

                // Debug: Show quantity before selling
                int itemCount = Rs2Inventory.count(itemName, true);
                Microbot.log("Found " + itemCount + " of this item to sell");

                // Try selling with different approaches
                boolean success = false;

                // First try with exact matching
                Microbot.log("Attempting sell with exact matching...");
                success = attemptSellWithTimeout(itemName, percent, true);
                Microbot.log("Exact match attempt result: " + success);

                if (!success) {
                    Microbot.log("Failed to sell item: " + itemName);
                } else {
                    Microbot.log("Successfully submitted sell order for: " + itemName);

                    // Debug: Check if item is still in inventory after selling
                    int remainingCount = Rs2Inventory.count(itemName, true);
                    Microbot.log("Items remaining in inventory after sell: " + remainingCount);
                    if (remainingCount > 0) {
                        Microbot.log("WARNING: Items still in inventory - sell may have failed or only sold some items");
                    }
                }

                Microbot.log("Sleeping between items...");
                sleep(1200, 2000);
                Microbot.log("Sleep completed");

            } catch (Exception e) {
                Microbot.log("Exception while processing item " + item.getName() + ": " + e.getMessage());
                e.printStackTrace();
            }

            Microbot.log("=== FINISHED PROCESSING: " + item.getName() + " ===");
        }

        Microbot.log("=== SELL LOOT DEBUG END ===");
        boolean result = Rs2Inventory.items().noneMatch(item -> lootList.contains(item.getId()));
        Microbot.log("Final result: " + result);

        // Final inventory check
        Microbot.log("=== FINAL INVENTORY CHECK ===");
        Rs2Inventory.items().forEach(item -> {
            if (lootList.contains(item.getId())) {
                Microbot.log("Still in inventory: " + item.getName() + " (ID: " + item.getId() + ") qty: " + item.getQuantity());
            }
        });
    ...
    // Modified attemptSell with detailed step-by-step debugging
    public static boolean attemptSellWithTimeout(String itemName, int percent, boolean exact) {
        Microbot.log("=== ATTEMPT SELL: " + itemName + " (exact: " + exact + ") ===");

        try {
            // Step 1: Check if item exists in inventory (same check as processOffer does)
            Microbot.log("Step 1: Checking if item exists in inventory: '" + itemName + "' (exact: " + exact + ")");

            // Debug: List all inventory items to see what's available
            Microbot.log("Current inventory contents:");
            Rs2Inventory.items().forEach(invItem -> {
                Microbot.log("  - '" + invItem.getName() + "' (ID: " + invItem.getId() + ")");
            });

            if (!Rs2Inventory.hasItem(itemName, exact)) {
                Microbot.log("FAIL - Item not found in inventory: '" + itemName + "' (exact: " + exact + ")");

                // Try to find similar names
                Microbot.log("Looking for similar item names...");
                Rs2Inventory.items().forEach(invItem -> {
                    String invName = invItem.getName();
                    if (invName.toLowerCase().contains(itemName.toLowerCase()) ||
                            itemName.toLowerCase().contains(invName.toLowerCase())) {
                        Microbot.log("  Similar item found: '" + invName + "' vs '" + itemName + "'");
                    }
                });

                return false;
            }
            Microbot.log("PASS - Item confirmed in inventory");

            // Step 2: Check available slots (same check as processOffer does)
            Microbot.log("Step 2: Checking available GE slots...");
            int availableSlots = Rs2GrandExchange.getAvailableSlots().length;
            Microbot.log("Available slots count: " + availableSlots);
            if (availableSlots == 0) {
                Microbot.log("FAIL - No available slots");
                if (!Rs2GrandExchange.isAnyF2pSlotAvailable()) {
                    Rs2GrandExchange.abortOfferSmart(true);
                }
                return false;
            }
            Microbot.log("PASS - Available slots found");

            // Step 3: Get item count before selling
            int itemCount = Rs2Inventory.count(itemName, exact);
            Microbot.log("Step 3: Found " + itemCount + " items to sell");

            // Step 4: Manual sell attempt - mimic what processOffer does internally
            Microbot.log("Step 4: Attempting to interact with item in inventory...");
            if (!Rs2Inventory.interact(itemName, "Offer", exact)) {
                Microbot.log("FAIL - Could not interact with item for Offer");
                return false;
            }
            Microbot.log("PASS - Successfully interacted with item");

            // Step 5: Wait for offer screen
            Microbot.log("Step 5: Waiting for offer screen to open...");
            if (!Global.sleepUntil(() -> GrandExchangeWidget.isOfferTextVisible(), 5000)) {
                Microbot.log("FAIL - Offer screen did not open within timeout");
                return false;
            }
            Microbot.log("PASS - Offer screen opened");

            // Step 6: Set quantity to ALL (THIS IS THE KEY FIX)
            Microbot.log("Step 6: Setting quantity to ALL by putting integer 0");

            //Rs2GrandExchange.setQuantity(0);
            Microbot.log("Actually, why would that work? if automatically it puts ALL for noted items");

            sleep(500, 800);

            // Step 7: Adjust price if needed
            if (percent != 0) {
                Microbot.log("Step 7: Adjusting price by " + percent + "%");
                Rs2GrandExchange.adjustPriceByPercent(percent);
                sleep(500, 800);
            }

            // Step 8: Confirm the offer
            Microbot.log("Step 8: Confirming offer...");
            Rs2GrandExchange.confirm();

            // Step 9: Wait for offer screen to close
            Microbot.log("Step 9: Waiting for offer screen to close...");
            boolean success = Global.sleepUntil(() -> !Rs2GrandExchange.isOfferScreenOpen(), 5000);

            if (success) {
                Microbot.log("SUCCESS - Offer completed successfully");

                // Step 10: Verify items were actually sold
                sleep(1000); // Give it a moment
                int remainingItems = Rs2Inventory.count(itemName, exact);
                Microbot.log("Items remaining in inventory: " + remainingItems + " (started with: " + itemCount + ")");

                if (remainingItems >= itemCount) {
                    Microbot.log("WARNING - No items were sold! Offer may have failed or been placed as pending order");
                } else if (remainingItems > 0) {
                    Microbot.log("PARTIAL - Only some items were sold (" + (itemCount - remainingItems) + " sold, " + remainingItems + " remaining)");
                } else {
                    Microbot.log("COMPLETE - All items were sold successfully");
                }

            } else {
                Microbot.log("FAIL - Offer screen did not close, offer may have failed");
            }

            return success;

        } catch (Exception e) {
            Microbot.log("Exception in attemptSellWithTimeout for " + itemName + ": " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    /**
     * Checks all GE slots for offers that are outdated (price deviated > 10gp from current standard)
     * and aborts them to free up slots and capital.
     *
     * @return true if any stale offers were aborted
     */
    public static boolean abortStaleOffers() {
        Microbot.log("=== Checking for stale GE offers ===");

        // Get ABC Alch plugin for current prices
        AbcAlchPlugin plugin = null;
        try {
            plugin = (AbcAlchPlugin) Microbot.getPluginManager().getPlugins().stream()
                    .filter(p -> p.getClass().getName().equals(AbcAlchPlugin.class.getName()))
                    .findFirst()
                    .orElse(null);
        } catch (Exception e) {
            Microbot.log("Could not access ABC Alch plugin for price check");
            return false;
        }

        if (plugin == null || plugin.getAlchItems() == null) {
            Microbot.log("ABC Alch plugin not available - skipping stale offer check");
            return false;
        }

        boolean abortedAny = false;
        final int PRICE_DEVIATION_THRESHOLD = 10;

        GrandExchangeOffer[] offers = Microbot.getClient().getGrandExchangeOffers();
        if (offers == null) {
            Microbot.log("Could not retrieve GE offers");
            return false;
        }
        ...
        for (int i = 0; i < offers.length; i++) {
            ...
            // Only check in-progress BUY offers
            ...
            // Get the slot widget to retrieve item name
            GrandExchangeSlots slot = GrandExchangeSlots.values()[i];
            Widget parent = GrandExchangeWidget.getSlot(slot);
            if (parent == null) continue;

            // Get item name from slot
            Widget nameWidget = parent.getChild(19);
            if (nameWidget == null) continue;
            String itemName = nameWidget.getText();

            // Get offer price from slot
            Widget priceWidget = parent.getChild(25);
            if (priceWidget == null) continue;

            String priceText = priceWidget.getText();
            int offerPrice = 0;
            try {
                priceText = priceText.replaceAll("[^0-9]", "");
                offerPrice = Integer.parseInt(priceText);
            } catch (Exception e) {
                Microbot.log("Could not parse offer price for " + itemName);
                continue;
            }

            // Find current standard price from ABC Alch
            int currentStandardPrice = 0;
            for (AlchItem item : plugin.getAlchItems()) {
                if (item.getName().equalsIgnoreCase(itemName)) {
                    currentStandardPrice = item.getGePrice();
                    break;
                }
            }

            if (currentStandardPrice == 0) {
                Microbot.log("Could not find current price for " + itemName + " - skipping");
                continue;
            }

            // Calculate deviation
            int priceDeviation = Math.abs(offerPrice - currentStandardPrice);

            Microbot.log("Slot " + (i + 1) + ": " + itemName +
                    " | State: " + state +
                    " | Progress: " + traded + "/" + total +
                    " | Offer: " + offerPrice + "gp" +
                    " | Current: " + currentStandardPrice + "gp" +
                    " | Deviation: " + priceDeviation + "gp");

            // Abort if price deviated too much
            if (priceDeviation > PRICE_DEVIATION_THRESHOLD) {
                Microbot.log("⚠ STALE OFFER DETECTED: " + itemName +
                        " (offer " + offerPrice + "gp vs current " + currentStandardPrice + "gp)");
                Microbot.log("Aborting stale offer in slot " + (i + 1) + "...");

                NewMenuEntry menuEntry = new NewMenuEntry(
                        "Abort offer", "", 2, MenuAction.CC_OP, 2, parent.getId(), false);

                Rectangle bounds = parent.getBounds() != null &&
                        Rs2UiHelper.isRectangleWithinCanvas(parent.getBounds())
                        ? parent.getBounds()
                        : Rs2UiHelper.getDefaultRectangle();
                ...
                Rs2GrandExchange.collectAllToInventory1();
                sleep(1200, 1800);
                Rs2GrandExchange.collectAllToBank1();
                sleep(1200, 1800);

                abortedAny = true;
                Microbot.log("✓ Aborted stale offer for " + itemName);
            }
        }

        if (!abortedAny) {
            Microbot.log("No stale offers detected");
        }

        return abortedAny;
    }

	// Helper method for sleepUntil with timeout
	private static boolean sleepUntil(Supplier<Boolean> condition, long timeoutMs) {
	...
	// Alternative approach - process items one at a time with manual checks
	private static boolean attemptSell(String itemName, int percent, boolean exact) {
		Microbot.log("=== ATTEMPT SELL: " + itemName + " (exact: " + exact + ") ===");
		...
		Microbot.log("Checking inventory for: " + itemName + " (exact: " + exact + ")");
		if (!Rs2Inventory.hasItem(itemName, exact)) {
			Microbot.log("Item not found in inventory: " + itemName + " (exact: " + exact + ")");
			return false;
		}
		Microbot.log("Item confirmed in inventory");

		// Double-check GE is still open
		if (!Rs2GrandExchange.isOpen()) {
			Microbot.log("GE closed during processing, reopening...");
			if (!Rs2GrandExchange.openExchange()) {
				Microbot.log("Failed to reopen GE");
				return false;
			}
			sleep(1000, 1500);
		}
		...
		Microbot.log("Created GE request - calling processOffer...");
		boolean result = Rs2GrandExchange.processOffer(request);
		Microbot.log("processOffer returned: " + result);
		return result;
	}
