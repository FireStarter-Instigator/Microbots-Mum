...
public class Rs2Camera {
    ...
    public static void turnTo(final TileObject tileObject) {
        ...
        // Use a more natural range with slight bias towards center
        int randomMaxAngle = Rs2Random.fancyNormalSample(25, 55);
        setAngle(angle, randomMaxAngle);

        // 10% chance to make a small correction
        if (Rs2Random.dicePercentage(10)) {
            Rs2Random.wait(100, 400, Rs2Random.EWaitDir.wdMean);
            int correctionAngle = Rs2Random.between(30, 45);
            setAngle(angle, correctionAngle);
        }
    }
    ...
    public static void setAngle(int targetDegrees, int maxAngle) {
        ...
        Microbot.getClient().setCameraSpeed(2f);

        // Randomly decide to overshoot/undershoot (20% chance to overshoot)
        int adjustedMaxAngle;
        if (Rs2Random.dicePercentage(20)) {
            // Overshoot by 5-15 degrees
            adjustedMaxAngle = maxAngle - Rs2Random.between(5, 15);
        } else {
            adjustedMaxAngle = maxAngle;
        }

        if (getAngleTo(targetDegrees) > adjustedMaxAngle) {
            Rs2Keyboard.keyHold(KeyEvent.VK_LEFT);
            Global.sleepUntilTrue(() -> Math.abs(getAngleTo(targetDegrees)) <= adjustedMaxAngle, 50, 5000);
            Rs2Keyboard.keyRelease(KeyEvent.VK_LEFT);
        } else if (getAngleTo(targetDegrees) < -adjustedMaxAngle) {
    ...
    public static int getZoom() {
        return Microbot.getClient().getVarcIntValue(VarClientID.CAMERA_ZOOM_BIG);
    }
