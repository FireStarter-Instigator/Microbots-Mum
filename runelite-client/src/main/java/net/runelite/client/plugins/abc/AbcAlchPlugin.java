package net.runelite.client.plugins.abc;
...
import net.runelite.client.plugins.abc.model.Spell;
import net.runelite.client.plugins.abc.model.TickCounter;
import net.runelite.client.plugins.abc.model.XpTracker;
...
public class AbcAlchPlugin extends Plugin {
    ...
    /**
     * Checks if an item should be filtered out based on its name.
     * Filters trimmed armor, castle wars variants, and other cosmetic items.
     *
     * @param itemName The name of the item
     * @return true if the item should be hidden, false if it should be shown
     */
    private boolean shouldFilterItem(String itemName) {
        if (itemName == null || !config.filterUselessItems()) {
            return false;
        }

        String nameLower = itemName.toLowerCase();

        // Filter trimmed variants (g) and (t)
        if (nameLower.contains("(g)") || nameLower.contains("(t)")) {
            return true;
        }

        // Filter ALL castle wars armor variants (h1) through (h5)
        if (nameLower.contains("(h1)") || nameLower.contains("(h2)") ||
                nameLower.contains("(h3)") || nameLower.contains("(h4)") ||
                nameLower.contains("(h5)")) {
            return true;
        }

        // Filter ornament kit items
        if (nameLower.contains("(or)")) {
            return true;
        }

        // Filter gilded items
        if (nameLower.contains("gilded")) {
            return true;
        }

        // Filter 3rd age items
        if (nameLower.contains("3rd age") || nameLower.contains("third age")) {
            return true;
        }

        // Filter specific problematic items
        if (nameLower.contains("broken") ||
                nameLower.contains("cursed") ||
                nameLower.contains("decorative") ||
                nameLower.contains("castle wars")) {
            return true;
        }
    ...
    public void updatePriceList() {
        ...
        for (ItemPrice price : itemPrices) {
            ...
            // NEW: Filter out useless items like trimmed armor, h3/h4/h5, etc.
            if (shouldFilterItem(name)) {
                continue;
            }
            ...
            tempAlchItems.add(new AlchItem(name, itemId, gePrice, highAlchPrice, highAlchProfit, geLimit, isMembers, image));
        }
