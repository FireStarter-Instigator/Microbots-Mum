...
public class Rs2Widget {
    ...
    public static boolean clickWidgetSafe(Widget widget, int param0, int identifier, BooleanSupplier successCondition) {
        // Try fast click
        clickWidgetFast(widget, param0, identifier);

        // Check if the click worked
        if (!Global.sleepUntil(successCondition, 1200)) {
            Microbot.log("Fast click failed, using mouse click fallback");
            Microbot.getMouse().click(widget.getBounds());
            return Global.sleepUntil(successCondition, 1500);
        }
