...
public class Rs2GameObject {
    ...
    public static boolean canReach(net.runelite.api.coords.WorldPoint target, int objectSizeX, int objectSizeY, int pathSizeX, int pathSizeY) {
        if (target == null) return false;

        // Assuming Rs2Player.getRs2WorldPoint() returns a wrapper that has .pathTo()
        // and that it accepts the standard API target.
        List<net.runelite.api.coords.WorldPoint> path = Rs2Player.getRs2WorldPoint().pathTo(target, true);

        if (path == null || path.isEmpty()) return false;

        // Create centered WorldAreas using fully qualified Microbot WorldArea
        net.runelite.api.coords.WorldPoint pathEndpoint = path.get(path.size() - 1);

        // Calculate South-West coordinate for path area
        int pathSwX = pathEndpoint.getX() - pathSizeX / 2;
        int pathSwY = pathEndpoint.getY() - pathSizeY / 2;

        // Create Microbot WorldArea for the path endpoint
        net.runelite.client.plugins.microbot.util.api.coords.WorldArea pathArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        pathSwX, pathSwY, pathSizeX, pathSizeY, pathEndpoint.getPlane());

        // Calculate South-West coordinate for object area
        int objSwX = target.getX() - (objectSizeX + 2) / 2;
        int objSwY = target.getY() - (objectSizeY + 2) / 2;

        // Create Microbot WorldArea for the target object
        net.runelite.client.plugins.microbot.util.api.coords.WorldArea objectArea =
                new net.runelite.client.plugins.microbot.util.api.coords.WorldArea(
                        objSwX, objSwY, objectSizeX + 2, objectSizeY + 2, target.getPlane());
    ...
    private static boolean clickObject(TileObject object) {
        Microbot.log("getting into click object");
        return clickObject(object, "");
    }

    private static boolean clickObject(TileObject object, String action) {
        Microbot.log("object is?" + object);
        ...
        try {
            ...
            Microbot.log("hitting the invoke");
...
// MenuEntryImpl(getOption=Use, getTarget=Barrier, getIdentifier=43700, getType=GAME_OBJECT_THIRD_OPTION, getParam0=53, getParam1=51, getItemId=-1, isForceLeftClick=true, getWorldViewId=-1, isDeprioritized=false)
    ...
    /**
     * Returns true if the object is reachable from the player.
     * Will return false if a door blocks the path.
     *
     * @param gameObject target object
     * @return boolean
     */
    public static boolean isReachable1(GameObject gameObject) {
        WorldArea worldArea = getWorldArea(gameObject);
        if (worldArea == null) {
            return false;
        }
        ...
        WorldPoint walkableInteractPoint = interactablePoints.stream()
                ...
                .filter(tile -> !isDoorBlocking(Microbot.getClient().getLocalPlayer().getWorldLocation(), tile))
                .findFirst()
                .orElse(null);
    ...
    /**
     * Checks if there is a blocking door object between the player and a target tile.
     */
    private static boolean isDoorBlocking(WorldPoint from, WorldPoint to) {
        // Get all game objects along the line between player and target
        List<Tile> tilesOnPath = Rs2Walker.getLineTiles(from, to);
        for (Tile tile : tilesOnPath) {
    ...
    /**
     * Determines if a given GameObject is a closed door.
     */
    private static boolean isClosedDoor(GameObject obj) {
        // Must use ObjectDefinition to get the name
        ObjectComposition def = Microbot.getClient().getObjectDefinition(obj.getId());
        String name = def != null ? def.getName() : null;
        if (name == null) return false;
        return name.toLowerCase().contains("door")
